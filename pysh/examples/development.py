...
def test_lint():
    ...
    pylint_plugins = json.loads(os.environ.get("PYSH_PYLINT_PLUGINS", '[]'))
    plugin_args = (
        ['--load-plugins'] + pylint_plugins) if pylint_plugins else []
...
def test(u: (bool, "Just unit tests for the package")=False):
    ...
    if pylint_plugins:
        os.environ["PYSH_PYLINT_PLUGINS"] = json.dumps(pylint_plugins)
